...
//    FOnOwnerDrawGrid: TJvDesignOwnerDrawGridEvent;
  ...
  public
    ...
    property DesignRoot: TComponent read FDesignRoot write FDesignRoot; //maurog
  published
    property Container: TWinControl read FContainer write SetContainer;
...
begin
  ...
  begin
    //OWNER FIX
    if Assigned(FDesignRoot) then
      C := CC.Create(FDesignRoot)
    else
      C := CC.Create(Container); // fallback

    //Name auch vom Root ableiten!
    if Assigned(FDesignRoot) then
      C.Name := DesignUniqueName(FDesignRoot, AddClass)
    else
      C.Name := DesignUniqueName(Container, AddClass);
    ...
    begin
      CO := TControl(C);
      if Assigned(FDesignRoot) then
        CO.Parent := TWinControl(FDesignRoot)
      else
        CO.Parent := SelectedContainer;
      CO.BoundsRect := GetBounds;
      Select(CO);
